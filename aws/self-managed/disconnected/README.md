...
- To populate operators from the mirror registry, run the following commands from the bastion node after deploying a cluster.
```bash
...
1. Install Advanced Cluster Management and Multicluster Engine operators from the mirror registry.

2. Set `hcp_cluster_suffixes` in `terraform.tfvars` **before** running
   `terraform apply`:
...
4. All values are automatically passed to Ansible via `ansible-vars.json`.
...
4. Make sure that the system trusts the ingress controller self signed CA so that the update service can pull the graph image via the route without unknown certifcate authority errors.
```bash
oc get secret router-ca -n openshift-ingress-operator -o jsonpath='{.data.tls\.crt}' | base64 -d > ingress-ca.crt
oc create configmap update-service-ca -n openshift-config --from-file=ca-bundle.crt=ingress-ca.crt
oc patch proxy/cluster --type=merge -p '{"spec":{"trustedCA":{"name":"update-service-ca"}}}'
```
Wait for pods to restart after this change.

5. Apply registry-trust.yaml to trust the mirror registry CA by the Update Service if not already done.
```bash
oc apply -f registry-trust.yaml
```
Both #3 and #4 can be skipped if an official CA signed certificate is used for Ingress Controller and Mirror Registry like LetsEncrypt.

7. From GUI of the main cluster install Operator `OpenShift Update Service`.

8. Apply `updateService` CR generated by `oc mirror`.
```bash
oc apply -f oc-mirror-output/working-dir/cluster-resources/updateService.yaml -n openshift-update-service
```

9. Obtain the policy engine route
```bash
...
```
10. Set the patch for the pull graph data using the POLICY_ENGINE_GRAPH_URI obtained in the previous step.
```bash
PATCH="{\"spec\":{\"upstream\":\"${POLICY_ENGINE_GRAPH_URI}\"}}"
```

11. Patch the CVO to use the OpenShift Update Service in your environment
```bash
oc patch clusterversion version -p $PATCH --type merge
```

12. From `OpenShift Console` -> `Administration` -> `Cluster Settings` -> `Details` view, see the update graph and trigger the update to the newly mirrored version.

13. If you cannot see the updated graph, wait till `cvo` does next polling or delete the `cluster-version-operator` pod to see the updated graph immediately.
```bash
